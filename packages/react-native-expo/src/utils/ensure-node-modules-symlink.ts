...
function getPackageJsonDependency(packageJsonPath: string): { [name: string]: string } {
...
function getProjectSymlynkDependency(packageJsonPath: string): Array<{ name: string, version: string }> {
  const dependencies = getPackageJsonDependency(packageJsonPath)
  return Object.keys(dependencies).map(depName => ({ name: depName, version: dependencies[depName] }));
}
 ...
 */
function stopIfIsExpoScopedPackageWithVersionStar(scope: string, name: string, version: string, basePackageVersion: string) {
  if (scope.startsWith('@expo')) {
    if (version === '*') {
      throw new Error(
        `Invalid version for @expo scoped package ${scope}/${name}. Change "${chalk.red(`${scope}/${name}`)}": "*" --> "${
          chalk.blue(`${scope}/${name}": "${basePackageVersion}`)
        }" Inside your application's package.json`
      );
    } else if(version !== basePackageVersion) {
      console.log(chalk.yellow(
        `Inside the package.json of your application the package ${scope}/${name} use version ${
          version
        } should be using ${basePackageVersion} as in your base package.json`
      ))
    }
  }
}